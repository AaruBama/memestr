...
function DropdownComponent() {
    ...
    return (<div>
            <Menu as="div" className="relative inline-block text-left">
                ...
                <div>
                    <Menu.Button
                        className="inline-flex justify-center rounded-full w-full text-sm font-medium border-solid-2 border-bg-black text-white bg-blue-500 border border-gray-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-100 focus:ring-indigo-500">
                        {((userDetails && userDetails.picture === undefined) || !userDetails) ? (
                            ...
                            </svg>) : <img src={userDetails.picture} alt="..."
                ...
                >
                    <Menu.Items
                        className="absolute p-4 right-0 w-40 mt-2 origin-top-right bg-black/[0.9] text-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                        ...
                        {isLoggedIn && <Menu.Item>
                            {({active}) => (<div
                                    class={`${active && 'bg-yellow-500'} ui-active:bg-white ui-active:text-black ui-not-active:bg-white ui-not-active:text-black`}>
                                    {userDetails.name}
                                </div>)}
                        </Menu.Item>}
                        {isLoggedIn && <Menu.Item
                        >
                            {({active}) => (<div onClick={() => {
                                    logout();
                                }} class={`${active && 'bg-yellow-500'} ui-active:bg-blue-500 ui-active:text-white ui-not-active:bg-white ui-not-active:text-black`}>
                                    Logout
                                </div>)}
                        </Menu.Item>
                        }
            ...
            <UserDetailsForAccountCreationModal isOpen={newUserDetailsModal} onClose={closeUserDetailModal} sk={sk}
                                                pk={pk}/>
