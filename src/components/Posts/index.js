...
function extractLinksFromText(text) {
  ...
  return links.filter(
    (link) => jpgRegex.test(link) || mp4Regex.test(link) || gifRegex.test(link)
  );
}
...
function Posts(props) {
  ...
  const [fillShare, setFillShare] = useState(false);
  const [showShareWindow, setShareWindow] = useState(false);
  const [timeDifference, setTimeDifference] = useState({
    unit: "",
    duration: 0,
  });

  function handleShareClick(_) {
    console.log("Fiill share", fillShare);
    setFillShare(!fillShare);
    setShareWindow(true);
  }

  function handleShareClose(_) {
  ...
  return (
    <>
      {showShareWindow && (
        <Share showShareWindow={showShareWindow} onClose={handleShareClose} />
      )}
        ...
        <div class="bg-gray-100 rounded-lg my-1 shadow-sm shadow-gray-400">
          ...
          <div class="flex align-items-center gap-x-3 bg-gray-100 border-b-4 border-white pl-2 mt-2">
            ...
            >
              <svg
                class={`${
                  fillZap && "fill-current text-yellow-300 stroke-black"
                } h-8 w-8`}
                xmlns="http://www.w3.org/2000/svg"
                x="0"
                y="0"
            ...
            >
              <svg
                className={`${fillLike && "fill-current text-red-600"} h-8 w-8`}
                xmlns="http://www.w3.org/2000/svg"
                x="0"
                y="0"
                fill="none"
            ...
            <button className="flex h-8 w-8" onClick={handleShareClick}>
              <svg
                class={`svg-icon ${fillShare && "fill-current text-blue-400"}`}
                viewBox="0 0 20 20"
                x="0"
                y="0"
                fill="none"
