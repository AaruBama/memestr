...
const UploadAndDisplayImage = ({setPicture}) => {
    ...
    return (
        <div className={"border-separate border-spacing-2 border border-solid border-2 border-b rounded-[10px] mt-4"} >
        <div class={"flex-nowrap bg-gray-500/[.06] px-2"}>
            ...
            <input
                ...
                onChange={(event) => {
                    ...
                    choosePicture(event.target.files[0]).then(r => {console.log(r)})
                }}
            />
            </div>
        </div>
